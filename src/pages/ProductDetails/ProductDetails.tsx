...
const ProductDetails: React.FC = () => {
  const { id } = useParams();
  const { data: product, isFetching } =  useGetProductQuery(`products/${id}`);
  ...
  return (
    <section>
      ...
      <div className="container product-details max-[767px]:flex-col max-[767px]:items-center max-[767px]:gap-6 pt-[100px] pb-[70px]">
        {/* Image - Left Side */}
        <div className="w-[522px] h-[580px] max-[1200px]:w-[450px] max-[1023px]:h-[498px] max-[1200px]:w-[330px] max-[1023px]:h-[365px] max-[575px]:h-auto max-[575px]:w-auto border p-[8px]">
        ...
        <div className="details pt-0 pr-0 pb-3 pl-8 flex-1 max-[575px]:pl-0">
          {/* Title */}
          <Link to={`${id}`} className="max-[1200px]:text-[22px] max-[767px]:text-[20px] max-[480px]:text-[18px] font-roboto font-medium text-[24px] leading-[29px] text-secondary_light cursor-pointer hover:text-primary transition duration-300">{product?.title}</Link>
          <div className="mt-2 flex items-center justify-between max-[500px]:flex-wrap">
            <div className="space-x-2 font-poppins">
              {/* Price */}
              <span className="text-primary text-[26px] leading-[39px] font-poppins font-bold">${product?.price}</span>
              <del className="font-poppins text-grey text-[14px]">${(product?.price+13).toFixed(2)}</del>
              <span className="text-[#388E3C] text-[14px]">35% Off</span>
            </div>
          {/* Stars */}
            <div className="mt-[6px] flex items-center">
              {Array.from({length: product?.rating.rate}).map((_, index) => <AiFillStar key={index} className="text-gold" />)}
              <span className="ml-2 text-grey text-[14px]">({product?.rating.count})</span>
            </div>
          </div>
          {/* Description */}
          <div className="mb-4 mt-2">
            <p className={`text-base leading-[28px] max-[480px]:text-[14px] text-grey mb-2 ${!showMore && "line-clamp-2"}`}>{product?.description}</p>
          ...
          {/* Functionalities */}
          <div className="max-[1023px]:flex-col max-[1023px]:items-start max-[767px]:flex-row max-[767px]:items-center max-[767px]:gap-4 max-[575px]:flex-col max-[575px]:items-start mt-4 flex items-center border-t border-b py-4">
            ...
            <div className="flex items-center space-x-3 mt-5 max-[767px]:mt-0">
              <button className="max-[1023px]:ml-0 main-btn rounded ml-4 max-[575px]:text-[14px] max-[575px]:py-[6px]">Add To Cart</button>
          ...
          {/* Categories */}
          <div className="mt-4">
            <p className="text-grey font-poppins">SKU : <span className="text-secondary_dark capitalize">&nbsp;{product?.id}</span></p>
            <p className="text-grey font-poppins mt-2">Category : <span className="text-secondary_dark capitalize">&nbsp;{product?.category}</span></p>
            <p className="text-grey font-poppins mt-2">Tags : <span className="text-secondary_dark">&nbsp;{product?.category.split(" ").join(', ').replace("'s", "")}</span></p>
      ...
      <div className="container related-products mb-[100px]">
        ...
        <div className="w-full mr-auto ml-auto py-10">
          <Slider {...settings}>
            {data?.filter((item) => item?.category == product?.category).filter((_, index) => index < 5).map((product) => <Product key={product?.id} {...product} />)}
          </Slider>
        </div>
      </div>
    </section>
